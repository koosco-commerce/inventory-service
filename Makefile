...
k8s-stop: ## Stop application (scale to 0)
	...
	@kubectl scale deployment/inventory-service -n commerce --replicas=0
	@kubectl scale deployment/inventory-service-mariadb -n commerce --replicas=0
...
k8s-start: ## Start application (scale to 2)
	...
	@kubectl scale deployment/inventory-service -n commerce --replicas=2
	@kubectl scale deployment/inventory-service-mariadb -n commerce --replicas=1
...
k8s-scale: ## Scale application (usage: make k8s-scale REPLICAS=3)
	...
	@kubectl scale deployment/inventory-service -n commerce --replicas=$(REPLICAS)
...
k8s-clean: ## Delete all Kubernetes resources
	...
	@kubectl delete deployment,svc,configmap,secret,hpa,pdb -n commerce -l app=inventory-service 2>/dev/null || true
	@kubectl delete deployment,svc,secret,pvc -n commerce -l app=inventory-service-mariadb 2>/dev/null || true
