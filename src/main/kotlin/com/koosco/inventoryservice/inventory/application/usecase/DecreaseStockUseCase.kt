...
@UseCase
class DecreaseStockUseCase(private val inventoryRepository: InventoryRepositoryPort) {

    @Transactional
    fun execute(command: DecreaseStockCommand) {
        inventoryRepository.tryDecreaseStock(
            command.skuId,
            command.reducingQuantity,
        ) || throw ConflictException(
            InventoryErrorCode.NOT_ENOUGH_STOCK,
            "재고가 부족합니다. skuId=${command.skuId}, reducingQuantity=${command.reducingQuantity}",
        )
    }

    @Transactional
    fun execute(command: BulkDecreaseStockCommand) {
        // 1. 모든 SKU ID 수집
