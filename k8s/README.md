# Kubernetes Deployment Guide for Inventory Service

## Overview
Complete Kubernetes configuration for the inventory-service microservice with MariaDB database.
...
kubectl exec deployment/mariadb -n commerce -- \
  mysqldump -uroot -proot commerce-inventory > backup.sql
```

### Restore Database
```bash
kubectl exec -i deployment/mariadb -n commerce -- \
  mysql -uroot -proot commerce-inventory < backup.sql
```

### Access Database Shell
```bash
kubectl exec -it deployment/mariadb -n commerce -- \
  mysql -uadmin -padmin1234 commerce-inventory
```
...
kubectl set image deployment/inventory-service \
  inventory-service=inventory-service:v2.0.0 -n commerce

# Watch rollout status:
kubectl rollout status deployment/inventory-service -n commerce
```

### Rollback
```bash
kubectl rollout undo deployment/inventory-service -n commerce

# Rollback to specific revision:
kubectl rollout undo deployment/inventory-service -n commerce --to-revision=2
```
...
# Test connection from inventory-service pod
kubectl exec -it deployment/inventory-service -n commerce -- \
...
```bash
# Recent logs
kubectl logs deployment/inventory-service -n commerce --tail=100

# Follow logs
kubectl logs -f deployment/inventory-service -n commerce

# Logs from all replicas
kubectl logs -l app=inventory-service -n commerce
```
