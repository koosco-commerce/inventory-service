...
echo -e "${GREEN}========================================${NC}"
echo -e "${GREEN}inventory Service Deployment${NC}"
...
kubectl wait --for=condition=ready pod -l app=inventory-service-mariadb -n ${NAMESPACE} --timeout=300s
...
# Pods
echo "Pods:"
kubectl get pods -n ${NAMESPACE} -l app=inventory-service

echo ""

# Services
echo "Services:"
kubectl get svc -n ${NAMESPACE} -l app=inventory-service
...
POD_NAME=$(kubectl get pod -n ${NAMESPACE} -l app=inventory-service -o jsonpath='{.items[0].metadata.name}')
...
echo ""
echo "Useful commands:"
echo "  - View logs: kubectl logs -f deployment/inventory-service -n ${NAMESPACE}"
echo "  - Port forward: kubectl port-forward svc/inventory-service 8080:80 -n ${NAMESPACE}"
echo "  - Get pods: kubectl get pods -n ${NAMESPACE}"
echo "  - Describe pod: kubectl describe pod <pod-name> -n ${NAMESPACE}"
echo ""
echo "Access application:"
echo "  - kubectl port-forward svc/inventory-service 8080:80 -n ${NAMESPACE}"
